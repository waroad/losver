...
class Model(nn.Module):
    def __init__(self, encoder, tokenizer, args,  num_labels=36):
    ...
    def forward(self, input_ids=None, labels=None, attention_mask=None, line_indices=None):
        ...
        line_weights = torch.ones((batch_size, seq_len), device=outputs.device)
                ...
                if line_indices[batch_idx]:
                    for line_idx, (start, end) in enumerate(line_boundaries):
                        if line_idx in line_indices[batch_idx]:
                            line_weights[batch_idx, start:end] = 5
        ...
        # Step 3: Apply weighted mean pooling
        weighted_outputs = outputs * attn_scores
        pooled_output = weighted_outputs.sum(dim=1) / (attn_scores.sum(dim=1) + 1e-9)
...
def convert_examples_to_features(js, tokenizer, args):
    ...
    return InputFeatures(
        ...
        target=label_idx,  # Multi-label vector
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (torch.tensor(self.examples[i].source_ids),
                ...
                torch.tensor(self.examples[i].target, dtype=torch.long))  # Multi-label output
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if results['f1_score'] > best_f1:
                        ...
                        with open('result_weighted.txt', 'a') as f:  # 'a' mode appends to existing content
                            f.write(f"Epoch: {idx}  Best f1: {round(best_f1, 4)}\n")
...
def main(fold_index):
    ...
    args.test_data_file = f"{args.localized_location+fold_index}/test.jsonl"
    args.eval_data_file = f"{args.localized_location+fold_index}/val.jsonl"
    args.train_data_file=f"{args.localized_location+fold_index}/train.jsonl"
    ...
    model = Model(base_model, tokenizer=tokenizer, args=args)
