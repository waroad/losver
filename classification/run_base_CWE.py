...
class Model(nn.Module):
    def __init__(self, encoder, config, tokenizer, args, num_labels=36):
        ...
        # Add classification layer for single-label classification
        self.classifier = nn.Linear(self.config.hidden_size, num_labels)

    def forward(self, input_ids=None, labels=None):
        # Step 1: Encoder output
        outputs = self.encoder(input_ids)[0]  # (batch, seq_len, hidden_size)
        pooled_output = outputs.mean(dim=1)  # (batch_size, hidden_size)

        # Step 3: Apply dropout and feed through classifier
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)  # (batch_size, num_labels)

        # Step 4: Apply softmax to get probabilities (for single-label classification)
...
def convert_examples_to_features(js, tokenizer, args):
    ...
    cwe_labels = ['CWE-119', 'others', 'CWE-20', 'CWE-399', 'CWE-125', 'CWE-264', 'CWE-200', 'CWE-189', 'CWE-416',
                  'CWE-190', 'CWE-362', 'CWE-476', 'CWE-787', 'CWE-284', 'CWE-254', 'CWE-310', 'CWE-415', 'CWE-732',
                  'CWE-404', 'CWE-19', 'CWE-79', 'CWE-59', 'CWE-17', 'CWE-400', 'CWE-772', 'CWE-269', 'CWE-22',
                  'CWE-835', 'CWE-369', 'CWE-18', 'CWE-287', 'CWE-285', 'CWE-704', 'CWE-617', 'CWE-134', 'CWE-358']

    # Get one-hot encoding
    cwe_id = js["CWE ID"]
    target = [0] * len(cwe_labels)  # Extract 0/1 for each CWE

    if cwe_id in cwe_labels:
        idx = cwe_labels.index(cwe_id)
        target[idx] = 1
        label_idx = target.index(1)

    return InputFeatures(source_tokens, source_ids, label_idx)
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):
        return torch.tensor(self.examples[i].input_ids), torch.tensor(self.examples[i].label, dtype=torch.long)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            ...
            labels.append(label_indices.cpu().numpy())

        nb_eval_steps += 1

        # Process results
    logits = np.concatenate(logits, 0)  # Shape: (n_samples, n_classes)
    labels = np.concatenate(labels, 0)  # Shape: (n_samples,)
...
def test(args, model, tokenizer):
    ...
    with open('result_base.txt', 'a') as f:  # 'a' mode appends to existing content
        f.write(f"{args.output_dir} {args.test_data_file} acc: {result['eval_accuracy']} f1: {result['f1_score']}, precision: {result['precision']}, recall: {result['recall']}\n")
...
def main(fold_index):
    ...
    set_seed(args.seed)
    args.output_dir+=fold_index
    args.train_data_file = args.train_data_file[:9] + fold_index + args.train_data_file[9:]
    args.eval_data_file = args.eval_data_file[:7] + fold_index + args.eval_data_file[7:]
    args.test_data_file = args.test_data_file[:8] + fold_index + args.test_data_file[8:]
