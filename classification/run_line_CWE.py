...
class Model(nn.Module):
    def __init__(self, encoder, config, tokenizer, args):
        ...
        self.drop_out = args.dropout_probability
    ...
    def forward(self, input_ids=None, labels=None):
        outputs = self.encoder(input_ids)[0]
        line_logits = []

        for i in range(input_ids.size(0)):  # Batch loop
            # Find line boundaries using newline token
            line_indices = (input_ids[i] == self.newline_token_id).nonzero(as_tuple=True)[0]
            ...
            for j in range(len(line_start_indices) - 1):
                ...
                if not end > start:
                    sample_line_logits.append(torch.tensor(-10.0, device=input_ids.device, requires_grad=True))
                    continue
            ...
            if sample_line_logits:
                ...
                line_logits.append(torch.zeros(1, dtype=torch.float, device=input_ids.device, requires_grad=True))
...
def convert_examples_to_features(js, tokenizer, args):
    ...
    return InputFeatures(source_tokens, source_ids, sparse_matrix)
...
def test(args, model, tokenizer):
    ...
    for k in kk:
        ...
        splits = re.search(r"(test|val|train)", args.test_data_file).group(1)
        if k == 0:
            add_label(args.test_data_file, f"{args.output_dir}/{splits}.jsonl", adjusted_preds, args)
