...
def test(args, model, tokenizer):
    ...
    input_lengths = []  # input 길이를 저장할 리스트 추가

    for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
        inputs = batch[0].to(args.device)
        label = batch[1].to(args.device)
        batch_lengths = (inputs != 1).sum(dim=1).cpu().numpy()
    ...
    with open(f'result_base.txt', 'a') as f:  # 'a' mode appends to existing content
        f.write(f"{args.output_dir} {args.test_data_file} Pc: {pc_ratio} Pv: {pv_ratio}  Pb: {pb_ratio}  Pr: {pr_ratio} acc: {accuracy} f1: {f1}, precision: {precision}, recall: {recall}\n")
