...
def collate_fn(batch):
    ...
    # Handle line indices - keep as list of lists
    # No need to convert to tensor as it will be processed in the model
...
def convert_examples_to_features(js, tokenizer, args):
    ...
    if args.use_ground_truth:
        critical = js['lines_ground_truth']
    else:
...
def train(args, train_dataset, model, tokenizer):
    ...
    best_mrr = 0.0
    best_acc, best_f1, best_pc = 0.0, 0.0, 0.0
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if results['f1_score'] > best_f1:
                        ...
                        torch.save(model_to_save.state_dict(), output_dir)
                        logger.info("Saving model checkpoint to %s", output_dir)
                    if results['pc_score'] > best_pc:
                        best_pc = results['pc_score']
                        logger.info("  " + "*" * 20)
                        logger.info("  Best pc:%s", round(best_pc, 4))
                        logger.info("  " + "*" * 20)

                        checkpoint_prefix = 'checkpoint-best-pc'
                        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
                        if not os.path.exists(output_dir):
                            os.makedirs(output_dir)
                        model_to_save = model.module if hasattr(model, 'module') else model
...
def evaluate(args, model, tokenizer):
    ...
    pair_correct = 0  # P-C: Correct prediction for both elements
    pair_vulnerable = 0  # P-V: Both predicted as vulnerable (incorrect)
    pair_benign = 0  # P-B: Both predicted as benign (incorrect)
    pair_reversed = 0  # P-R: Inversely predicted labels

    num_pairs = len(preds_binary) // 2

    for i in range(0, len(preds_binary), 2):
        vuln_pred = preds_binary[i]  # First element (vulnerable)
        clean_pred = preds_binary[i + 1]  # Second element (clean)
        vuln_label = labels_binary[i]  # Should be 1 (vulnerable)
        clean_label = labels_binary[i + 1]  # Should be 0 (clean)

        # Check if both predictions are correct
        if vuln_pred == vuln_label and clean_pred == clean_label:
            pair_correct += 1
        # Check if both predicted as vulnerable
        elif vuln_pred == 1 and clean_pred == 1:
            pair_vulnerable += 1
        # Check if both predicted as benign
        elif vuln_pred == 0 and clean_pred == 0:
            pair_benign += 1
        # Check if predictions are reversed
        elif vuln_pred == 0 and clean_pred == 1:
            pair_reversed += 1

    # Calculate pair-wise percentages
    pc_ratio = pair_correct / num_pairs
    pv_ratio = pair_vulnerable / num_pairs
    pb_ratio = pair_benign / num_pairs
    pr_ratio = pair_reversed / num_pairs
    result["pc_score"]=round(pc_ratio, 4)
    return result
...
def test(args, model, tokenizer):
    ...
    print(f"Pc: {pc_ratio} Pv: {pv_ratio}  Pb: {pb_ratio}  Pr: {pr_ratio}  Accuracy: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}")
    with open('result_weighted.txt', 'a') as f:  # 'a' mode appends to existing content
        if args.use_ground_truth:
            f.write(f"{args.output_dir} {args.test_data_file} Ground truth line used\n")
        else:
            f.write(f"{args.output_dir} {args.test_data_file} Predicted line used\n")
        f.write(f"Pc: {pc_ratio} Pv: {pv_ratio}  Pb: {pb_ratio}  Pr: {pr_ratio}  Accuracy: {accuracy:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}, F1: {f1:.4f}\n")
...
def main():
    ...
    parser.add_argument("--use_ground_truth", action='store_true',
                        help="Whether to run with ground truth modified lines")
    ...
    args.eval_data_file = f"{args.localized_location}/valid.jsonl"
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        checkpoint_prefix = 'checkpoint-best-pc/pytorch_model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
        model.load_state_dict(torch.load(output_dir))
        model.to(args.device)
