...
class Model(nn.Module):
    ...
    def forward(self, input_ids=None, labels=None):
        outputs = self.encoder(input_ids, attention_mask=input_ids.ne(1))[0]  # (B, L, H)
        ...
        if labels is not None:
            ...
            loss = torch.nn.functional.binary_cross_entropy(prob, labels)
            return loss, prob
        else:
            return prob
...
def test(args, model, tokenizer):
    ...
    cnt=0
    cnt1=0
    with open(f"./{args.output_dir}/{args.test_data_file[:-6]}.output", 'w') as f, open(f"./{args.output_dir}/{args.test_data_file[:-6]}.gold", 'w') as f1:
    ...
    with open(f'result_base{args.GPU}.txt', 'a') as f:  # 'a' mode appends to existing content
        f.write(f"{args.output_dir} {args.test_data_file} acc: {accuracy} f1: {f1}, precision: {precision}, recall: {recall}\n")
